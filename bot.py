...
def init_db():
    with sqlite3.connect('shop.db') as conn:
        c = conn.cursor()
        c.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, balance INTEGER DEFAULT 0, joined_date TEXT)')
        ...
        c.execute('CREATE TABLE IF NOT EXISTS custom_categories (name TEXT PRIMARY KEY)')
        c.execute('CREATE TABLE IF NOT EXISTS custom_products (id INTEGER PRIMARY KEY AUTOINCREMENT, category_name TEXT, name TEXT, price INTEGER)')
        
        try: c.execute('ALTER TABLE history ADD COLUMN purchase_time TEXT')
        except sqlite3.OperationalError: pass 
            
        try: c.execute('ALTER TABLE users ADD COLUMN joined_date TEXT')
        except sqlite3.OperationalError: pass
        ...
        if c.fetchone()[0] == 0:
            default_rules = (
                ...
                "**3. Предзаказ:** Если нужного номинала нет, вы можете оформить заказ. Среднее время выдачи — 15-30 минут.\n\n"
                "📞 **Поддержка:** По всем вопросам обращайтесь к нашему менеджеру: **@Djabrail050**"
            )
            c.execute('INSERT INTO settings (key, value) VALUES ("rules", ?)', (default_rules,))
...
def get_balance(user_id):
    with sqlite3.connect('shop.db') as conn:
        ...
        if result is None:
            ...
            c.execute('INSERT INTO users (id, balance, joined_date) VALUES (?, 0, ?)', (user_id, current_date))
            conn.commit()
            return 0
        return result[0]
...
def set_rules(new_rules):
    with sqlite3.connect('shop.db') as conn:
        c = conn.cursor()
        c.execute('UPDATE settings SET value = ? WHERE key = "rules"', (new_rules,))
        conn.commit()
...
def is_menu_button(text):
    ...
    if text in menu_buttons: return True
...
def admin_panel(message):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("💰 Цены (Вручную)", callback_data="admin_price_cats"),
    ...
    markup.add(
        InlineKeyboardButton("📂 Создать категорию", callback_data="admin_add_cat"),
    ...
    markup.add(
        ...
        InlineKeyboardButton("📝 Изменить правила", callback_data="admin_edit_rules")
    )
    bot.send_message(message.chat.id, "🛠 **Админ-панель магазина**", parse_mode="Markdown", reply_markup=markup)
...
def handle_text(message):
    ...
    if text == "🛒 Купить":
        ...
        with sqlite3.connect('shop.db') as conn:
            c = conn.cursor()
            c.execute('SELECT item_name, code, price, purchase_time FROM history WHERE user_id = ? ORDER BY rowid DESC LIMIT 30', (message.from_user.id,))
            rows = c.fetchall()
            if not rows: 
                bot.send_message(message.chat.id, "Вы еще ничего не покупали в нашем магазине 😔")
            else:
        ...
        bot.send_message(message.chat.id, "🚧 Раздел находится в разработке! Используйте добавление собственных категорий через админ-панель.")
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            if c.fetchone():
                c.execute('SELECT id, name, price FROM custom_products WHERE category_name = ?', (text,))
                products = c.fetchall()
                if not products:
                    bot.send_message(message.chat.id, "В этой категории пока нет товаров. Скоро появятся!")
                    return
                ...
                for p in products:
                    ...
                    markup.add(InlineKeyboardButton(f"{p[1]} ({p[2]} ₽) | {status}", callback_data=f"buy_custom_{p[0]}"))
                markup.add(InlineKeyboardButton("❌ Закрыть", callback_data="close_menu"))
                
                bot.send_message(message.chat.id, f"📦 **Категория:** {text}\nВыберите товар:", reply_markup=markup, parse_mode="Markdown")
...
def handle_query(call):
    if call.data == "close_menu":
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(f"✅ Оформить заказ ({price} ₽)", callback_data=f"manualpay_apple_{currency}_{nom}"))
        markup.add(InlineKeyboardButton("❌ Отмена", callback_data=f"apple_curr_{currency}"))
        
        text = (f"🛒 **Оформление заказа**\n\nТовар: App Store & iTunes {nom} {currency}\nСтоимость: **{price} ₽**\n\n"
                f"📦 *Данный товар выдается после обработки заказа менеджером.*")
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            c.execute('INSERT INTO history (user_id, item_name, code, price, purchase_time) VALUES (?, ?, ?, ?, ?)', 
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            if product is None:
                return bot.answer_callback_query(call.id, "😔 Кто-то успел купить последний код перед вами. Оформите заказ!", show_alert=True)
            ...
            item_name = f"App Store & iTunes {nom} {currency}"
            c.execute('INSERT INTO history (user_id, item_name, code, price, purchase_time) VALUES (?, ?, ?, ?, ?)', 
                      (call.from_user.id, item_name, secret_code, final_price, current_time))
            conn.commit()
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            markup = InlineKeyboardMarkup()
            if stock > 0:
                markup.add(InlineKeyboardButton(f"💳 Оплатить {price} ₽", callback_data=f"pay_custom_{product_id}"))
                markup.add(InlineKeyboardButton("❌ Отмена", callback_data="close_menu"))
                text = f"🛒 **Подтверждение покупки**\n\nТовар: {prod_name}\nК списанию: **{price} ₽**"
            else:
                markup.add(InlineKeyboardButton(f"✅ Оформить заказ ({price} ₽)", callback_data=f"manualpay_custom_{product_id}"))
                markup.add(InlineKeyboardButton("❌ Отмена", callback_data="close_menu"))
                text = (f"🛒 **Оформление заказа**\n\nТовар: {prod_name}\nСтоимость: **{price} ₽**\n\n"
                        f"📦 *Данный товар выдается после обработки заказа менеджером.*")
        ...
        username = f"@{call.from_user.username}" if call.from_user.username else "Скрыт"
        admin_text = (f"🚨 **НОВЫЙ ЗАКАЗ (СВОЙ ТОВАР)!** 🚨\n\n👤 Пользователь: {username}\n🆔 ID: `{call.from_user.id}`\n"
                      f"🛒 Товар: **{prod_name}**\n💰 Оплачено: **{price} ₽**\n\n⚡️ Свяжитесь с клиентом и выдайте товар.")
        bot.send_message(ADMIN_ID, admin_text, parse_mode="Markdown")
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            if product_code is None:
                return bot.answer_callback_query(call.id, "😔 Товары закончились. Оформите заказ!", show_alert=True)
                
            code_id, secret_code = product_code
            c.execute('UPDATE users SET balance = balance - ? WHERE id = ?', (price, call.from_user.id))
            c.execute('DELETE FROM codes WHERE id = ?', (code_id,))
        ...
        bot.edit_message_text(f"✅ **Успешная покупка!**\n\nТовар: {prod_name}\nВаш товар/код:\n`{secret_code}`", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
        bot.send_message(ADMIN_ID, f"💰 Пользователь ID `{call.from_user.id}` купил {prod_name} за {price} ₽ (Автовыдача)", parse_mode="Markdown")
    ...
    # ============ АДМИН ПАНЕЛЬ ============
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            c.execute('SELECT count(*) FROM users WHERE id != ? AND joined_date = ?', (ADMIN_ID, today))
            today_count = c.fetchone()[0]
            
            c.execute('SELECT count(*) FROM users WHERE id != ? AND joined_date LIKE ?', (ADMIN_ID, this_month))
        ...
        markup = InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ Назад", callback_data="back_to_admin"))
        ...
        msg = bot.send_message(call.message.chat.id, "📂 Отправьте название новой категории (например: Roblox 👾):")
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            markup = InlineKeyboardMarkup(row_width=2)
            for cat in cats:
                markup.add(InlineKeyboardButton(cat[0], callback_data=f"adm_selcat_{cat[0]}"))
            bot.edit_message_text("📦 Выберите категорию для нового товара:", call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data.startswith("adm_selcat_"):
        cat_name = call.data.replace("adm_selcat_", "")
        msg = bot.send_message(call.message.chat.id, f"Категория: **{cat_name}**\n📝 Отправьте название нового товара:", parse_mode="Markdown")
        ...
        bot.edit_message_text("Выберите валюту (Ручное изменение):", call.message.chat.id, call.message.message_id, reply_markup=markup)
        ...
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("Турция (TRY)", callback_data="admin_rate_curr_TRY"), 
            InlineKeyboardButton("США (USD)", callback_data="admin_rate_curr_USD"),
            InlineKeyboardButton("Европа (EUR)", callback_data="admin_rate_curr_EUR"), 
            InlineKeyboardButton("Индия (INR)", callback_data="admin_rate_curr_INR")
        )
        bot.edit_message_text("💱 Выберите валюту для обновления цен по курсу:", call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data.startswith("admin_rate_curr_"):
        currency = call.data.split("_")[3]
        msg = bot.send_message(call.message.chat.id, f"Выбрана валюта: **{currency}**\n\nОтправьте новый курс (сколько рублей стоит 1 {currency}):\n*(Например: 100 или 2.5)*", parse_mode="Markdown")
        ...
        markup.add(InlineKeyboardButton("🌟 Свои товары", callback_data="admin_code_custom_cats"))
        bot.edit_message_text("Выберите раздел для загрузки кода:", call.message.chat.id, call.message.message_id, reply_markup=markup)
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            markup = InlineKeyboardMarkup(row_width=2)
            for cat in cats:
                markup.add(InlineKeyboardButton(cat[0], callback_data=f"adm_code_selcat_{cat[0]}"))
            markup.add(InlineKeyboardButton("⬅️ Назад", callback_data="admin_add_code"))
            bot.edit_message_text("Выберите категорию своих товаров:", call.message.chat.id, call.message.message_id, reply_markup=markup)
            
    elif call.data.startswith("adm_code_selcat_"):
        cat_name = call.data.replace("adm_code_selcat_", "")
        with sqlite3.connect('shop.db') as conn:
            c = conn.cursor()
            c.execute('SELECT id, name FROM custom_products WHERE category_name = ?', (cat_name,))
            prods = c.fetchall()
            markup = InlineKeyboardMarkup(row_width=1)
            for p in prods:
                markup.add(InlineKeyboardButton(p[1], callback_data=f"admin_code_item_custom_{p[0]}"))
            markup.add(InlineKeyboardButton("⬅️ Назад", callback_data="admin_code_custom_cats"))
            bot.edit_message_text("Выберите товар для загрузки кода:", call.message.chat.id, call.message.message_id, reply_markup=markup)
        ...
        msg = bot.send_message(call.message.chat.id, f"🔑 Отправьте код/товар для **{item_key}** (он будет выдан клиенту после оплаты):", parse_mode="Markdown")
        ...
        msg = bot.send_message(call.message.chat.id, "Отправьте новый текст для **Правил магазина**:", parse_mode="Markdown")
        bot.register_next_step_handler(msg, process_edit_rules)

    elif call.data.startswith("topup_paid_"):
        amount = int(call.data.split("_")[2])
        bot.edit_message_text(f"Вы указали, что перевели {amount} ₽.", call.message.chat.id, call.message.message_id)
        msg = bot.send_message(call.message.chat.id, f"📸 Пожалуйста, отправьте **фотографию чека** или скриншот перевода в этот чат:", parse_mode="Markdown")
        ...
        try: bot.send_message(int(call.data.split('_')[3]), "❌ Ваше пополнение было отклонено менеджером. Проверьте чек или обратитесь в поддержку.")
        except: pass
...
def process_add_category(message):
    ...
    with sqlite3.connect('shop.db') as conn:
        c = conn.cursor()
        try:
            c.execute('INSERT INTO custom_categories (name) VALUES (?)', (message.text,))
            conn.commit()
            bot.send_message(message.chat.id, f"✅ Категория **{message.text}** добавлена! Она появится в меню 'Купить'.", parse_mode="Markdown")
...
def process_add_prod_name(message, cat_name):
    ...
    msg = bot.send_message(message.chat.id, f"Товар: **{prod_name}**\n💰 Отправьте цену товара в рублях (только число):", parse_mode="Markdown")
...
def process_add_prod_price(message, cat_name, prod_name):
    ...
    try:
        ...
        with sqlite3.connect('shop.db') as conn:
            c = conn.cursor()
            c.execute('INSERT INTO custom_products (category_name, name, price) VALUES (?, ?, ?)', (cat_name, prod_name, price))
            conn.commit()
        bot.send_message(message.chat.id, f"✅ Товар **{prod_name}** за **{price} ₽** успешно добавлен в категорию **{cat_name}**!", parse_mode="Markdown")
    except ValueError:
    ...
    try:
        ...
        with sqlite3.connect('shop.db') as conn:
            ...
            for nom in APPLE_DENOMINATIONS[currency]:
                ...
                c.execute('UPDATE prices SET price = ? WHERE item_key = ?', (new_price, item_key))
            conn.commit()
        bot.send_message(message.chat.id, f"✅ Курс для **{currency}** установлен: **{new_rate} ₽**.\nВсе цены в этой категории успешно пересчитаны и обновлены!", parse_mode="Markdown")
    except ValueError:
        bot.send_message(message.chat.id, "❌ Ошибка. Введите число (например: 100 или 2.5).")

def process_receipt_photo(message, amount):
    if message.text and is_menu_button(message.text): return handle_text(message)
    
    if message.content_type != 'photo':
        bot.send_message(message.chat.id, "❌ Вы не отправили фотографию. Пополнение отменено, попробуйте снова через меню.")
        return
        
    photo_id = message.photo[-1].file_id
    bot.send_message(message.chat.id, f"⏳ Чек на {amount} ₽ отправлен. Ожидайте проверки менеджером.")
    
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton("✅ Выдать", callback_data=f"adm_topup_ok_{message.from_user.id}_{amount}"), 
        InlineKeyboardButton("❌ Отклонить", callback_data=f"adm_topup_no_{message.from_user.id}")
    )
    caption = f"🔔 **ПРОВЕРКА ОПЛАТЫ!**\n👤 ID: `{message.from_user.id}`\n💰 Сумма: **{amount} ₽**\n\nВнимательно проверьте чек перед выдачей:"
...
def process_save_new_code(message, item_key):
    ...
    with sqlite3.connect('shop.db') as conn:
        ...
        conn.commit()
    bot.send_message(message.chat.id, f"✅ Товар/Код успешно загружен в базу!", parse_mode="Markdown")
...
def process_add_balance(message):
    ...
    except: bot.send_message(message.chat.id, "❌ Ошибка. Формат: ID СУММА")

def process_edit_rules(message):
    if is_menu_button(message.text): return handle_text(message)
    set_rules(message.text)
    bot.send_message(message.chat.id, "✅ Правила магазина успешно обновлены!", parse_mode="Markdown")
